...
if( typeof googletag === "undefined" ){ // todo: check this syntax
...
wbads = (function($, window, document, undefined) {
    'use strict';
    ...
    var module_defaults  = {
        ...
        debug_enabled:      false,
        device_view:        "main",
        selector:           "tpixads-container",
        data_store:         "tpixadData",
        evt_callbacks:      {
            onBeforeInit:           null,
            onAfterInit:            null,
            onBeforeEnableServices: null,
            onAfterEnableServices:  null,
            onBeforeSlotDiscovery:  null,
            onAfterSlotDiscovery:   null,
            onBeforeDefineSlot:     null,
            onAfterDefineSlot:      null,
            onBeforeDisplayAds:     null,
            onAfterDisplayAds:      null
        },
        quantcast:          {
            enabled:                true,
...
//        debug("Slot :: creating AD SLOT OBJECT with id:" + id);
...
//        debug("Slot :: AD SLOT OBJECT created with id:" + id);
    }
...
//        debug("setTargetingParam :: setting param[" + key + "] = " + value + "...");
    ...
    function init(site_id, site_domain, ad_zone, options, dfp_options) {
        ...
        if(!settings.enabled) {
            debug("init :: TPIXADS ads disabled. no ad slots will be created or displayed");
            return;
        }

        /** bind any passed-in/default callbacks before any events can fire */
        $.each(settings.evt_callbacks, function(name, func) {
            debug("init :: binding callback defined for " + name + "...");
        ...
        googletag.cmd.push( function() {
            ...
            if( dfp_settings.enable_async ) googletag.pubads().enableAsyncRendering();
    ...
    function buildSlots() {
        ...
        if(!settings.enabled) {
            debug("buildSlots :: TPIXADS ads disabled. no ad slots will be created or displayed");
            return;
        }

        debug("buildSlots :: TPIXADS ad container discovery...");
        /** find all tpixads ad containers on the page */
        ...
        debug("buildSlots :: ...TPIXADS ad container discovery found and created " + dfpContainersCount + " ads");
    ...
    function addSlot(divId, sizeType, sizeList, interstitial, refresh) {
        if(!settings.enabled) {
            debug("addSlot :: TPIXADS ads disabled. no ad slots will be created or displayed");
            return false;
        }
     ...
     * @param {string} slotDivId - the DOM id of the slot to update (eg. "dfp-tpixad-1")
...
//        debug("setRequiredParam :: param " + param + " set to: " + required_params[param]);
    ...
    function setEnabled(showAds) {
        ...
        debug("setEnabled :: TPIXADS ads enabled : " + settings.enabled);
    }
    /**
     * get if enables/disables ads
     * @return {boolean}
     */
    function getEnabled() {
    ...
    function setDeviceView(device) {
        ...
        debug("setDeviceView :: TPIXADS device_view set to : " + settings.device_view);
    }
...
//        debug("getParamFromUri :: checking for param " + param + " from page url...");
...
var tpixAdsManager = wbads; // legacy support for a minute